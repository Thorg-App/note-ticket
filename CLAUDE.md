...
- `ticket.ts` — `Ticket` entity: typed field accessors, immutable `withField`/`withoutField`, `toJsonRecord()` (the `query` payload), `TicketField` (the on-disk key names, one place), `TicketStatus` (the union of statuses this CLI WRITES — `Ticket.status` stays `string` because disk text is arbitrary; text becomes a `TicketStatus` only via `TicketStatusArgument.parsed`; `TicketStatus` = built-ins | branded `CustomTicketStatus`)
...
- `custom-ticket-status.ts` — `CustomTicketStatus` (user-named status like `p2`) + `CustomTicketStatusParser` (the ONE validation rule). No behavior of its own: `DepGraph` treats only `open`/`in_progress` as active
...
- `choice-argument.ts` — `ChoiceArgument`: the ONE rule for an argument that must be one of fixed literals (`invalid <noun> '<x>'. Must be one of: …` + the `Valid <nouns>:` usage tail). `TicketProfileArgument` (`commands/profile.ts`) is an instance; `TicketStatusArgument` (`commands/status.ts`) wraps one for the built-ins and also admits custom statuses

...
- `choice-argument.ts` — `ChoiceArgument`: the ONE rule for an argument that must be one of fixed literals (`invalid <noun> '<x>'. Must be one of: …` + the `Valid <nouns>:` usage tail). `TicketStatusArgument` (`commands/status.ts`) and `TicketProfileArgument` (`commands/profile.ts`) are its two instances

...
/**
 * A ticket file's text split into its frontmatter block and its body, able to
 * reproduce the original bytes exactly when nothing was changed.
 */
export class TicketDocument {
    private constructor(
        ...
        private readonly hasBlock: boolean,
    ) {}
    ...
    static parse(text: string): TicketDocument {
        ...
        if (openIndex < 0) {
            return new TicketDocument(Frontmatter.EMPTY, [], lines, false);
        }
        const closeOffset = lines.indexOf(FRONTMATTER_MARKER, openIndex + 1);
        const blockEnd = closeOffset < 0 ? lines.length : closeOffset;
        return new TicketDocument(
            Frontmatter.fromLines(lines.slice(openIndex + 1, blockEnd)),
            lines.slice(0, openIndex),
            lines.slice(Math.min(blockEnd + 1, lines.length)),
            true,
        );
    }

    static of(frontmatter: Frontmatter, body: string): TicketDocument {
        return new TicketDocument(frontmatter, [], body.split(LINE_SEPARATOR), true);
    }

    /** Body text after the closing marker, verbatim. */
    body(): string {
        return this.bodyLines.join(LINE_SEPARATOR);
    }

    withFrontmatter(frontmatter: Frontmatter): TicketDocument {
        return new TicketDocument(frontmatter, this.prologue, this.bodyLines, true);
    }

    /** Append text to the body, keeping exactly one trailing newline. */
    withBodyAppended(text: string): TicketDocument {
        return TicketDocument.of(this.frontmatter, this.body() + text);
    }

    /** Full file text. Byte-identical to the input when nothing was changed. */
    text(): string {
        if (!this.hasBlock) {
            return this.bodyLines.join(LINE_SEPARATOR);
        }

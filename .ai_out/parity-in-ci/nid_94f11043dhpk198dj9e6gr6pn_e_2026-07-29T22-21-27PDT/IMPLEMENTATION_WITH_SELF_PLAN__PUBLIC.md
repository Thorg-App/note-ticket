...
- **No apt install step.** `python3`, `git` and `jq` are preinstalled on `ubuntu-latest`, and `uv`
  ...
  present and aborts non-zero if it is not — so if the runner image ever drops jq, CI fails with a
  message naming jq instead of a confusing `rc=127, expected 141` plus a silently-thinned
  `_check_jsonl` (corrected in round 1: it would have gone red either way).
...
| jq guard is not a no-op | ran `run.py` with a PATH symlink farm excluding only `jq` → exit **1** naming jq (message reworded in round 1) |

> Round 1 (iteration) added: corrected `require_jq()` rationale, a per-invocation `min_lines`
> vacuity guard in `check_query`, the T6 ticket breadcrumb, `if: ${{ !cancelled() }}`,
> `timeout-minutes: 20`, and a CI mention in CLAUDE.md. See `IMPLEMENTATION_ITERATION__PUBLIC.md`.
